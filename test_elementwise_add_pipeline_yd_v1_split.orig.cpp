...
extern "C" __global__ __aicore__ void main_kernel( GM_ADDR A_handle,  GM_ADDR B_handle,  GM_ADDR C_handle, uint64_t fftsAddr) {
  ...
  if ASCEND_IS_AIV {
    ...
    for (int32_t n_i = 0; n_i < 6; ++n_i) {
      ...
      AscendC::Add(a_ub[0], a_ub[((n_i % 2) * 2048)], b_ub[((n_i % 2) * 2048)], 2048);
      ...
      tl::ascend::copy_ub_to_gm<float, 32, 64>(C[(((((cid / 4) * 131072) + (vid * 65536)) + ((cid % 4) * 256)) + (n_i * 32))], a_ub[0], 1024);
    }
    ...
    AscendC::Add(a_ub[0], a_ub[2048], b_ub[2048], 2048);
    AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(4);
    AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(4);
    AscendC::PipeBarrier<PIPE_MTE3>();
    tl::ascend::copy_ub_to_gm<float, 32, 64>(C[(((((cid / 4) * 131072) + (vid * 65536)) + ((cid % 4) * 256)) + 224)], a_ub[0], 1024);
  }
}
